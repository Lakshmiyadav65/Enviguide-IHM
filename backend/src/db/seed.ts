...
// Vessels assigned to specific users (by email)
const vesselAssignments = [
  {
    ownerEmail: 'narasimha.goggi@enviguide.com',
    vessels: [
      {
  ...
  {
    ownerEmail: 'sivaprasad@enviguide.com',
    vessels: [
      {
  ...
  {
    ownerEmail: 'saisanjan.kethamreddy@enviguide.com',
    vessels: [
      {
...
async function seed() {
  ...
  // Seed vessels with ownership
  for (const assignment of vesselAssignments) {
    const ownerId = userMap[assignment.ownerEmail];
    for (const v of assignment.vessels) {
      const vessel = await prisma.vessel.upsert({
        where: { imoNumber: v.imoNumber },
        update: { ...v, createdById: ownerId },
        create: { ...v, createdById: ownerId },
      });
      console.log(`Seeded vessel: ${vessel.name} (IMO ${vessel.imoNumber}) -> owner: ${assignment.ownerEmail}`);
    }
  }
}
